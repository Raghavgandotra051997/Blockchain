...
chaincode_name  := env_var_or_default("WORKSHOP_CHAINCODE_NAME",     "EMRSmartContract")
...
debugcc:
    ...
    export CHAINCODE_ID=$(weft chaincode package caas --path . --label EMRSmartContract --address ${CHAINCODE_SERVER_ADDRESS} --archive EMRSmartContract.tgz --quiet)
    export CORE_PEER_LOCALMSPID=org1MSP
    export CORE_PEER_MSPCONFIGPATH=$CFG/_msp/org1/org1admin/msp
    export CORE_PEER_ADDRESS=org1peer-api.127-0-0-1.nip.io:8080
    export CORE_PEER_CLIENT_CONNTIMEOUT=15s
    ...
    set -x && peer lifecycle chaincode install EMRSmartContract.tgz &&     { set +x; } 2>/dev/null
    echo
    set -x && peer lifecycle chaincode approveformyorg --channelID mychannel --name EMRSmartContract -v 0 --package-id $CHAINCODE_ID --sequence 1 --connTimeout 15s && { set +x; } 2>/dev/null
    echo
    set -x && peer lifecycle chaincode commit --channelID mychannel --name EMRSmartContract -v 0 --sequence 1  --connTimeout 15s && { set +x; } 2>/dev/null
    echo
